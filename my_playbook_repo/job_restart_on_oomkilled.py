...
def job_restart_on_oomkilled(event: JobEvent, params: IncreaseResources):
    ...
    try:
        ...
        logging.error(
            f"get_job_latest_pod was called on event without job's pod: {event}"
        )
    ...
    for ind, status in enumerate(pod.status.containerStatuses):
        if status.state.terminated.reason == oom_killed:
            ...
            status_flag = True
            break
    # Extracting request['memory'] from the containers
    max_res = PodContainer.get_requests(
    ...
    if status_flag:
        ...
        finding.add_enrichment(
            [
                MarkdownBlock(f"*The job's pod was not killed because of OOM*"),
            ]
        )
        event.add_finding(finding)
    ...
    for container in containers_spec:
        containers_list.append(
            Container(
                ...
                envFrom=container.envFrom,
                imagePullPolicy=container.imagePullPolicy,
                resources=increase_resource(
                    container.resources, increase_by, max_resource
                )
                if (container.resources.limits and container.resources.requests)
                else None,
            )
